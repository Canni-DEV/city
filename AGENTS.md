...
- The product is local-only: no telemetry, server dependency, accounts, or economy. A bounded runtime pedestrian layer (M3.6 / M3.6.1 / SIM-001–010) and a bounded runtime vehicle layer (M3.6.2 / SIM-011–019) are in scope; massive crowds and massive traffic are not.

## Scope boundaries

- Do not modify source files under `assets/`.
- Do not add elevated roads, road editing, zoning editing, PWA support, audio, massive crowd or massive vehicle simulation, or heavy post-processing in v1.
