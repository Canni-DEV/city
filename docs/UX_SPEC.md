...
- **UX-011:** Right-drag orbits, wheel zooms, and Q/E rotates the camera. Default framing still shows the whole city; OrbitControls `maxZoom` is raised above 28 in M3.6 so streets and agents can be inspected. **F** toggles unrestricted free flight only outside NPC control. M3.8 adds an explicit perspective `npcFollow` mode while controlling a selected NPC; Tab returns to city orbit and Escape releases control, clears selection, and returns to city orbit.
...
- **UX-027:** Every visible NPC has an enlarged invisible pick target and stable-ID selector. Selection highlights the avatar and announces accessible status without changing autonomy/camera. Explicit Control enters camera-relative WASD/arrow control and `npcFollow`; Shift runs, V greets, X/Stop holds, Tab releases while preserving selection, Escape releases and clears, and F is ignored during control. Keyboard handling excludes editable controls and clears held keys on blur or mode changes. Development home exposes an Animation lab card; production does not.
