...
- **UX-001:** The library is the entry point; the city route combines a dominant 3D viewport with generation and diagnostic controls. Visible chrome uses laboratory labels (Generate, Parameters, Seed, Size), not product marketing or editor voice. M3.9 adds an unlinked optional technical showcase and does not redirect or replace the library.
...
- **UX-028:** The M3.9 showcase uses semantic English HTML and an inline vector `City` wordmark above a fixed 3D canvas. The city begins clipped to the wordmark's `i` dot; five viewports of native, reversible scroll expand the mask and move a non-interactive perspective camera through overview, district and elevated one-block/intersection frames. A deterministic stopped NPC greets once and exposes `Meet me`; activating it crossfades to locked follow control. Back to library remains visible. Below 1280×720 the route shows a desktop notice without generation; reduced-motion desktop users receive a static final composition and `Explore the city`.
...
UX-028 visual refinement: retain floating narrative cards with compact 13–16px body text, muted labels, lighter borders and reduced blur. Place cards 24px from the outer side and 28px from the top/bottom to leave the central cinematic view clear. Copy and scroll timing are unchanged.
