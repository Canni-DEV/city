...
- **REN-011:** Runtime pedestrians mount one non-physics `AnimatedCharacter` per stable NPC ID, sized to catalog height `dimensions[1] * uniformScale` and visually lifted by the existing sidewalk slab offset. Fixed ticks capture actual pose/velocity and render frames interpolate with the shared clock alpha. Selection uses a transparent enlarged pick target and lightweight highlight. `npcFollow` is a 50° damped perspective orbit-follow camera targeting the interpolated actor root. The development animation lab is lazy and production output contains no reachable lab or Rapier chunk; City rendering never initializes Rapier.
