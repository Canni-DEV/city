...
M3.8 (ADR-0019) absorbs procedural animation into `@city/procedural-animation`. Core adds runtime-only control, social and animation-directive components while retaining exclusive motion/navigation/collision/crossing authority and no Three.js dependency. The animation package owns rig posing and optional bounded motion requests. Web owns actor registration, interpolation, accessible selection/input and the explicit `cityOrbit` / `freeFlight` / `npcFollow` camera modes. M3.8.1 makes `npcFollow` a locked third-person rig and adds deterministic yield recovery in `tickNpcWorld`. The package `/physics` entry remains optional and is not reachable from the production City application.

M3.9 (ADR-0020) adds an optional hybrid DOM/R3F experience route. It shares the validated generation worker and `CityScene`, but keeps its generated document local until explicit adoption. Native scroll and a route-owned perspective camera are web-only; the handoff carries a validated transient NPC ID and reconstructs all disposable runtime state from the adopted document.
