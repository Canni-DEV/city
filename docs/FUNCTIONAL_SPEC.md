...
- **FUN-048:** Every runtime NPC uses procedural idle/walk/run/greet animation. Every visible NPC can be selected by viewport or stable ID and explicitly controlled; manual movement, Shift running, greeting, stopping, release and full-leg crossing runs remain inside core navigation/collision/traffic authority. Control is transient and never mutates `CityDocumentV1`.
