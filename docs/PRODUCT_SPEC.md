...
- **PRD-006:** The product has no goals, economy, vehicle traffic, accounts, server dependency, telemetry, onboarding, or PWA behavior. A bounded runtime pedestrian layer (SIM-001–007) is allowed; massive crowds are not.
...
- **AC-004:** M0–M6, intermediate M3.5/M3.6, their mapped requirements, and all manual QA checks are complete before `1.0.0`.

## Out of scope for v1

Massive crowd or vehicle simulation, user-controlled avatars, leaving the road graph, road editing, zoning editing, bridges, ramps, multilevel networks, multiplayer, cloud sync, mobile-first controls, seasons, time of day, audio, and heavy post-processing.
