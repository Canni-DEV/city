...
M3.7.3 adds no schema fields. Yard and courtyard props are ordinary `CityEntity` records. Generator `0.9.0` cities include them in the structural hash via `entities`, with unique-hash lot dumpsters and sub-cell transforms on jittered occupants. Old `0.8.1` cities remain loadable without silent regeneration and simply lack composed yards.

M3.8 adds no schema field, migration, generator stage, catalog entry, or structural-hash input. NPC control, social cooldowns, animation directives, actor registrations, selected runtime ID and camera mode exist only in reconstructed runtime state and are discarded on document open/regeneration. Existing `0.8.1` and `0.9.0` documents remain loadable without regeneration.
