...
This supersedes the M3.6 / ADR-012 walk policy that bound agents to occupied road cells. It does not replace ADR-003, ADR-006, ADR-011, or ADR-012’s rule that agents are outside `CityEntity` / export. Generator version is `0.6.3`; lots shrink by the ring; Kenney street meshes keep their baked curb (the extra cell reads as a wide sidewalk). Avenue T/4-way cells use unsuffixed Kenney junction meshes so connectors match topology; Kenney `*-line` is not used. Vehicle traffic remains out of v1. M4 must not treat sidewalk tiles as selectable objects.
