...
This supersedes the M3.6 / ADR-012 walk policy that bound agents to occupied road cells. It does not replace ADR-003, ADR-006, ADR-011, or ADR-012’s rule that agents are outside `CityEntity` / export. Generator version is `0.6.6` after local 4-way roundabouts (ADR-011 amendment); lots shrink by the ring; Kenney street meshes keep their baked curb (the extra cell reads as a wide sidewalk). Dual T/4-way 2×2 cells with four openings use `road-crossroad`. Kenney `*-line` is not used. A bounded runtime vehicle layer is specified in ADR-014. M4 must not treat sidewalk tiles as selectable objects.
