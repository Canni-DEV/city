...
Coverage is complete without manually transcribing every city-kit model. Exceptions remain reviewable and build validation prevents stale paths, duplicate IDs, bad footprints, and incomplete connectors. ADR-0012 adds generated protagonist GLBs outside `/assets`; it does not rewrite this 213-GLB city-kit decision. ADR-014 adds measured `driveProfile` and `vehicleBounds` through the same override-and-regenerate path.
