...
Correction verification: pnpm check (existing warnings only), pnpm typecheck, pnpm test (119 core, 36 web, 25 animation and asset suite), pnpm build and git diff --check pass. In-app Chromium visual smoke at 1920x1080 and 1280x720 confirms dot alignment, immediate native-scroll enlargement, reverse to scale 1, loading recovery and the final Meet me composition. The animation loop now remains mounted across scroll events. Standalone Chrome/Edge and forced WebGL 2 owner matrix remains pending.
...
Card refinement verification: pnpm check, pnpm typecheck, pnpm test, pnpm build and git diff --check pass. In-app browser smoke confirms compact opening and rules cards remain at the outer edges without covering the central wordmark/aperture.
