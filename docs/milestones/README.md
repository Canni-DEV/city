# Milestone briefs

Each brief is an execution contract. Complete only its scope, attach its required evidence to the pull request, update the requirements matrix, and stop for review.

- [M0 — Foundation](M0_FOUNDATION.md)
- [M1 — Road vertical slice](M1_ROAD_VERTICAL_SLICE.md)
- [M2 — Blocks and zoning](M2_BLOCKS_AND_ZONING.md)
- [M3 — Placement and rendering](M3_PLACEMENT_AND_RENDERING.md)
- [M3.5 — Urban morphology](M3_5_URBAN_MORPHOLOGY.md)
- [M3.6 — Animated agents](M3_6_ANIMATED_AGENTS.md)
- [M3.6.1 — Pedestrian sidewalks](M3_6_1_SIDEWALKS.md)
- [M3.6.2 — Runtime vehicles](M3_6_2_VEHICLES.md)
- [M3.6.3 — NPC refactor](M3_6_3_NPC_REFACTOR.md)
- [M4 — Editor](M4_EDITOR.md)
- [M5 — Local library](M5_LOCAL_LIBRARY.md)
- [M6 — Hardening and 1.0](M6_HARDENING.md)
